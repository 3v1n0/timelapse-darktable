...
public class DTParameter extends LinkedHashMap<String,Object> {
	...
	public String write() {
		...
		for (int i=0 ; i<this.length ; i++) {
			...
			} else if ( this.get("type").equals("int") || this.get("type").equals("boolean") ) {
			...
			// inverts order of the 8 char word, 2 by 2 char : DDCCBBAA => AABBCCDD (little/big endian convention?)
			param = param + s.substring(6,8) + s.substring(4,6) + s.substring(2,4) + s.substring(0,2);
		}
		return param;
	}
