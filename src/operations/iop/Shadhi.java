...
/*
 * from darktable typedef struct dt_iop_shadhi_params_t { dt_gaussian_order_t
 * order; float radius; float shadows; float reserved1; float highlights; float
 * reserved2; float compress; float shadows_ccorrect; float highlights_ccorrect;
 * } dt_iop_shadhi_params_t;
 * 
 * typedef enum dt_gaussian_order_t { DT_IOP_GAUSSIAN_ZERO = 0,
 * DT_IOP_GAUSSIAN_ONE = 1, DT_IOP_GAUSSIAN_TWO = 2 } dt_gaussian_order_t;
 * 
 * 
 * typedef struct dt_iop_shadhi_data_t { dt_gaussian_order_t order; float
 * radius; float shadows; float highlights; float compress; float
 * shadows_ccorrect; float highlights_ccorrect; } dt_iop_shadhi_data_t;
 */
