...
/*
 * 
 * typedef struct dt_iop_basecurve_node_t { float x; float y; }
 * dt_iop_basecurve_node_t;
 * 
 * typedef struct dt_iop_basecurve_params_t { // three curves (c, ., .) with max
 * number of nodes // the other two are reserved, maybe we'll have cam rgb at
 * some point. dt_iop_basecurve_node_t basecurve[3][MAXNODES]; // MAXNODES=20
 * int basecurve_nodes[3]; int basecurve_type[3]; } dt_iop_basecurve_params_t;
 */