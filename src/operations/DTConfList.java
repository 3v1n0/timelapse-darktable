...
public class DTConfList extends TreeSet<DTConfiguration>  {
	...
	public double[][] todoubleMat(ArrayList<Number[]> numList){
		// variable with more than one dimension (vector) are put in one matrix (jVec,iList)
		// doubleArray[jVec] is one vector composed of all iList parameters
		double[][] doubleArray = new double[numList.get(1).length][numList.size()];
		for (int iList = 0; iList < numList.size(); iList++) {
			for (int jVec = 0; jVec < numList.get(0).length; jVec++) {
				doubleArray[jVec][iList] = numList.get(iList)[jVec].doubleValue();
			}
		}
		return doubleArray;
	}
	...
	public double[] genInterpIndexList() {
		...
		double[] xi = new double[(1+ (int) Math.abs(indSrc[indSrc.length-1] - indSrc[0]))];
	...
	public double[] interpSplineOneOpParamAllIndices(String operation,String parameter,int paramIndex) {
		// just for test 
